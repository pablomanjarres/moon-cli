 ...
 *
 * Indexes start at 0, so --red=0 is the first letter. A dash also works
 * (--red-0) but '=' is the conventional way to hand a value to a long option.
 *
                      ...
                      const char **why)
{
    ...
    /* Split the name from the number. '=' is the usual way to attach a value to a
     * long option (--jobs=4), so we look for that first; a dash is accepted too. */
    const char *body = flag + 2;              /* "red=0" or "red-0"        */
    const char *sep  = strchr(body, '=');
    if (sep == NULL)
        sep = strrchr(body, '-');             /* no '=', so the LAST dash  */
    ...
    /* Whatever follows the separator has to be a plain, sane number.
     *
    ...
    size_t name_len = (size_t)(sep - body);
    for (int i = 0; i < NUM_NAMED; i++) {
